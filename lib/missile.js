class Missile {
  constructor(ctx, xPos, angle, speed) {
    ...
    this.fallingSound = new Audio([
      "assets/sound/104551__chimerical__bomb-whistle-long.wav"
    ]);
    this.falling = false;
  }
  ...
  draw() {
    ...
    if (this.cityHit) {
    ...
    } else if (this.generalHit) {
      // (!this.collided && !this.exploded){
      ...
      this.ctx.drawImage(
        ...
        60,
        50
      ); //adjusts image to have tip be at at xpos,ypos
  ...
  createAngles(xPos, yPos, height, width) {
    ...
    return leftSideAngles
      .concat(topAngles)
      .concat(rightSideAngles)
      .concat(bottomAngles);
  }

  xCheck(coord, otherObject) {
    if (
      coord[0] > otherObject.x &&
      coord[0] < otherObject.x + otherObject.width
    ) {
      return true;
    } else {
      return false;
  ...
  yCheck(coord, otherObject) {
    if (
      coord[1] > otherObject.y &&
      coord[1] < otherObject.y + otherObject.height
    ) {
      return true;
    } else {
      return false;
